<?php
final class MyMapStylingPage {

	public static function indexPage($css_content) {
	?>
		<div class="wrap">
		<h2>My Mapped Locations - Styling</h2>
		...
		<div class="inside">
		  <form action="" method="post">
		  	<p>
		  		...
		  		<select id="css_file_selection" name="css_file_selection">
		  			<?php
		  			if ($handle = opendir(get_template_directory()."/my_mapped_locations_css")) {
					    /* This is the correct way to loop over the directory. */
					    while (false !== ($entry = readdir($handle))) {
					        if (preg_match("/\.css$/", $entry)) {
					        	$selected = "";
					        	if (get_option("my_mapped_location_current_css_file") == $entry) {
					        		$selected = "selected";
					        	}
		  ...
		  <p>If you run into any issues with the My Mapped Locations stylesheet, you can reset them by clicking on the reset button below. You will lose your current css changes though, so make sure you do a backup.</p>
		  <form action="" method="post">
